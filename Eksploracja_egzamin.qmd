...
1. Czy dane są MAR (ang. Missing At Random) czy MNAR (ang. Missing Not At Random) (@sec-missings), co oznacza, że musimy się zastanowić jakie mogły być źródła braków danych, przypadkowe czy systematyczne?
...
## 4. Czym są braki MCAR, MAR, MNAR? {#sec-missings}
...
- <u>porządkowe</u> - posiadające skończoną liczbę stanów z zachowaniem porządku (np. wykształcenie)
...
<u>Niewystarczające dopasowanie</u> - sytuacja w której parametry dopasowania modelu pokazują  słabe dopasowanie, zarówno na zbiorze uczącym, jak i testowym. Takie modele również nie są użyteczne w pozyskiwaniu wiedzu na temat badanego zjawiska, a sytuację taką nazywamy niedouczeniem (ang. underfitting).
...
Przycinanie minimalizujące błąd opiera się na spostrzeżeniu, że błąd drzewa przyciętego charakteryzuje się zbyt pesymistyczną oceną (szacowaną na zbiorze testowym) i dlatego wymaga korekty. Węzeł drzewa klasyfikacyjnego $n$ zastępujemy liściem $l$, jeśli $$\hat e_T(l)\leq \hat e_T(n)$$ gdzie <br>
...
<p style="color:#808080">Estymatory wyznaczone na podstawie niewielkiej próby, mogą być obarczone znaczącym błędem. Wyliczanie błędu średnio-kwadratowego dla podzbioru nowych wartości może się charakteryzować takim obciążeniem. Dlatego stosuje się statystyki opisowe z poprawką, której pochodzenie może mieć trzy źródła: wiedza merytoryczna na temat szukanej wartości, założeń modelu lub na podstawie wyliczeń opartych o cały zbiór wartości. <br> <br> <https://dax44.github.io/datamining/drzewa-decyzyjne.html#przycinanie-minimalizuj%C4%85ce-b%C5%82%C4%85d></p>
